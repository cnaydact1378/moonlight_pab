...
def load_data():
    """실행 위치가 달라도 이 Python 파일 옆의 CSV를 찾는다."""
    try:
        candidates = sorted(
        ...
        st.error(f"프로젝트 폴더를 읽을 수 없습니다: {exc}")
        return None, None

    if not candidates:
        st.error(f"CSV 파일을 찾을 수 없습니다. {CSV_NAME} 파일을 대시보드.py와 같은 폴더에 넣어 주세요.")
        st.caption(f"확인한 폴더: {BASE_DIR}")
        return None, None

    preferred = BASE_DIR / CSV_NAME
    if preferred in candidates:
        path = preferred
    elif len(candidates) == 1:
        path = candidates[0]
        st.info(f"기본 파일명 대신 발견된 CSV를 사용합니다: {path.name}")
    else:
        # 여러 파일 중 임의로 하나를 골라 잘못 분석하지 않는다.
        name = st.sidebar.selectbox("분석할 CSV 파일", [path.name for path in candidates])
        path = BASE_DIR / name

    for encoding in ("utf-8-sig", "cp949", "utf-16"):
        try:
            # ID의 앞자리 0을 보존하고, 숫자/날짜는 별도로 변환한다.
            data = pd.read_csv(path, encoding=encoding, dtype="string")
            data.columns = data.columns.str.strip().str.lstrip("\ufeff")
            if data.columns.duplicated().any():
                st.error("공백을 제거한 뒤 중복되는 열 이름이 있습니다. CSV의 열 이름을 확인해 주세요.")
                return None, path
            return data, path
        except UnicodeError:
            continue
        except pd.errors.EmptyDataError:
            st.error("CSV가 비어 있습니다. 열 이름과 검사 데이터를 넣어 주세요.")
            return None, path
        except (pd.errors.ParserError, OSError, ValueError) as exc:
            st.error(f"CSV를 읽지 못했습니다. 파일의 구분자와 접근 권한을 확인해 주세요. ({exc})")
            return None, path
    st.error("CSV 인코딩을 확인하지 못했습니다. UTF-8 또는 CP949 CSV로 저장해 주세요.")
    return None, path
...
def show_charts(data):
    ...
    with right:
        ...
        if not summary.empty:
            st.bar_chart(summary, x="라인", y="불합격 건수", sort="-불합격 건수",
                         color="#fb923c", height=300)
...
def main():
    ...
    st.caption(f"데이터 파일: {path.name} · 원본 검사 {len(data):,}건 · 화면을 새로고침하면 CSV를 다시 읽습니다.")
    if messages:
